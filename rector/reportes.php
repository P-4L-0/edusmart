...
if($grupo_id && $materia_id) {
    ...
    foreach($estudiantes as $estudiante) {
        // Obtener actividades y calificaciones del trimestre
        $db->query("SELECT a.id, a.nombre, a.porcentaje, n.calificacion 
                   FROM actividades a 
        ...
        foreach($actividades as $actividad) {
            if($actividad->calificacion !== null) {
                $calificacion = ($actividad->calificacion * $actividad->porcentaje) / 100;
                ...
                $calificaciones[] = [
                    ...
                    'calificacion_ponderada' => $calificacion
                ];
            }
        }
        
        $promedio = $total_porcentaje > 0 ? ($total_calificacion / $total_porcentaje) * 100 : 0;
...
<body class="bg-gray-100">
    <div class="flex">
        ...
        <div class="flex-1 p-8">
            ...
            <div class="bg-white p-6 rounded-lg shadow mb-6">
                ...
                <form action="reportes.php" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select id="grupo_id" name="grupo_id" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                            <option value="">Seleccionar Grupo</option>
                            <?php foreach($grupos as $grupo): ?>
                                <option value="<?php echo $grupo->id; ?>" <?php echo $grupo_id == $grupo->id ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($grupo->nombre); ?> - <?php echo htmlspecialchars($grupo->grado); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="trimestre" name="trimestre" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="1" <?php echo $trimestre == 1 ? 'selected' : ''; ?>>Primer Trimestre</option>
                            <option value="2" <?php echo $trimestre == 2 ? 'selected' : ''; ?>>Segundo Trimestre</option>
                            <option value="3" <?php echo $trimestre == 3 ? 'selected' : ''; ?>>Tercer Trimestre</option>
                        </select>
                    </div>
            ...
            <?php if(!empty($reporte) && isset($info_reporte)): ?>
                <div class="bg-white p-6 rounded-lg shadow mb-6">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h3 class="text-xl font-semibold">Reporte de Calificaciones</h3>
                            <p class="text-gray-600"><?php echo htmlspecialchars($info_reporte->materia_nombre); ?> - <?php echo htmlspecialchars($info_reporte->grupo_nombre); ?> <?php echo htmlspecialchars($info_reporte->grado); ?></p>
                            <p class="text-gray-600">Trimestre: <?php echo $trimestre; ?></p>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <?php if(!empty($reporte[0]['actividades'])): ?>
                                        <?php foreach($reporte[0]['actividades'] as $actividad): ?>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" title="<?php echo htmlspecialchars($actividad['actividad']); ?> (<?php echo $actividad['porcentaje']; ?>%)">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($item['estudiante']); ?></td>
                                    <?php foreach($item['actividades'] as $actividad): ?>
                                        <td class="px-6 py-4 whitespace-nowrap <?php echo $actividad['calificacion'] < 60 ? 'text-red-500' : ''; ?>">
                                            <?php echo $actividad['calificacion']; ?>
                                        </td>
                                    <?php endforeach; ?>
                                    <td class="px-6 py-4 whitespace-nowrap font-semibold <?php echo $item['promedio'] < 60 ? 'text-red-500' : ''; ?>">
                    ...
                    <div class="mt-6 pt-4 border-t">
                        <p class="text-lg font-semibold">Promedio General del Grupo: <span class="<?php echo $promedio_general < 60 ? 'text-red-500' : ''; ?>"><?php echo round($promedio_general, 2); ?></span></p>
                    </div>
                </div>
